...
var CustomHexMap = class {
  constructor(container, options = {}) {
    ...
    this.hexWidth = Math.sqrt(3) * this.options.hexRadius;
    this.hexHeight = 2 * this.options.hexRadius;
    ...
    console.log(`\u{1F3A8} Canvas created: ${this.canvas.width}x${this.canvas.height}`);
  ...
  offsetToPixel(q, r) {
    ...
    const pixelY = this.options.hexRadius * (Math.sqrt(3) / 2 * q + Math.sqrt(3) * r);
    return {
      x: pixelX * this.zoom + this.panX + this.options.width / 2,
      y: pixelY * this.zoom + this.panY + this.options.height / 2
    };
  }
  pixelToOffset(pixelX, pixelY) {
    const worldX = (pixelX - this.panX - this.options.width / 2) / this.zoom;
    const worldY = (pixelY - this.panY - this.options.height / 2) / this.zoom;
    const q = 2 / 3 * worldX / this.options.hexRadius;
    const r = (-1 / 3 * worldX + Math.sqrt(3) / 3 * worldY) / this.options.hexRadius;
    ...
    if (qDiff > rDiff && qDiff > sDiff) {
      return { x: -roundedR - roundedS, y: roundedR };
    } else if (rDiff > sDiff) {
      return { x: roundedQ, y: -roundedQ - roundedS };
    } else {
      return { x: roundedQ, y: roundedR };
    }
  }
  getHexAtPixel(pixelX, pixelY) {
    if (!this.mapData)
      return null;
  ...
  drawPlayerMarker(center) {
    const size = Math.max(8, 16 * this.zoom);
    this.ctx.save();
    this.ctx.beginPath();
  ...
  render() {
    ...
    for (const hex of this.mapData.hexes) {
      ...
      if (this.options.showCoordinates) {
        this.drawHexCoordinates(center, hex.x, hex.y);
      }
    }
    if (this.mapData.player_position) {
      console.log("\u{1F3AF} Drawing player at position:", this.mapData.player_position);
      const playerCenter = this.offsetToPixel(this.mapData.player_position.x, this.mapData.player_position.y);
      console.log("\u{1F3AF} Player screen coordinates:", playerCenter);
      this.drawPlayerMarker(playerCenter);
    } else {
  ...
  setMapData(mapData) {
    var _a, _b;
    console.log("\u{1F5FA}\uFE0F CustomHexMap: Setting map data with", ((_a = mapData == null ? void 0 : mapData.hexes) == null ? void 0 : _a.length) || 0, "hexes");
    console.log("\u{1F5FA}\uFE0F CustomHexMap: Player position:", mapData == null ? void 0 : mapData.player_position);
    if (((_b = mapData == null ? void 0 : mapData.hexes) == null ? void 0 : _b.length) > 0) {
  ...
  centerOnPlayer() {
    ...
    const q = this.mapData.player_position.x;
    const r = this.mapData.player_position.y;
  ...
  isAdjacentToPlayer(hex) {
    ...
    if (!((_a = this.mapData) == null ? void 0 : _a.player_position))
      return false;
    const playerPos = this.mapData.player_position;
    const hexQ = hex.x;
    const hexR = hex.y;
    const hexS = -hexQ - hexR;
    const playerQ = playerPos.x;
    const playerR = playerPos.y;
...
// src/hexcrawlView.ts
var VIEW_TYPE_HEXCRAWL = "hexcrawl-view";
var HexcrawlView = class extends import_obsidian.ItemView {
  ...
  getViewType() {
    return VIEW_TYPE_HEXCRAWL;
  }
  getDisplayText() {
    return "Hexcrawl Explorer";
  }
  getIcon() {
    return "map";
  }
  async onOpen() {
    console.log("\u{1F3AF} Hexcrawl View: Opening view");
    const container = this.containerEl.children[1];
    container.empty();
    container.addClass("hexcrawl-view");
    const mainContainer = container.createDiv({ cls: "hexcrawl-main" });
    this.createControls(mainContainer);
    const mapContainer = mainContainer.createDiv({ cls: "hexcrawl-map-container" });
    mapContainer.id = "hexcrawl-map";
    ...
    const infoPanel = mainContainer.createDiv({ cls: "hexcrawl-info" });
    this.createInfoPanel(infoPanel);
  }
  createControls(container) {
    const controls = container.createDiv({ cls: "hexcrawl-controls" });
    new import_obsidian.ButtonComponent(controls).setButtonText("Refresh Map").setIcon("refresh-cw").onClick(async () => {
      await this.refreshMap();
      if (this.hexMap) {
        this.hexMap.centerOnPlayer();
      }
      new import_obsidian.Notice("Map refreshed!");
    });
    new import_obsidian.ButtonComponent(controls).setButtonText("Center View").setIcon("crosshair").onClick(() => {
      if (this.hexMap) {
        this.hexMap.centerOnPlayer();
      }
    });
  ...
  createInfoPanel(container) {
    container.createEl("h3", { text: "Current Hex Information" });
    const infoContent = container.createDiv({ cls: "hex-info-content" });
    infoContent.id = "hex-info-content";
    infoContent.innerHTML = "<p>No hex selected</p>";
  }
  updateInfoPanel(content) {
    const infoContent = document.getElementById("hex-info-content");
    if (infoContent) {
      infoContent.innerHTML = `<div class="hex-description">${content}</div>`;
    }
  }
  initializeMap(container) {
    console.log("\u{1F5FA}\uFE0F Hexcrawl View: Initializing custom hex map on container:", container.id);
    try {
      this.hexMap = new CustomHexMap(container, {
        hexRadius: 25,
        showGrid: this.plugin.settings.showHexGrid,
        showCoordinates: this.plugin.settings.showHexGrid
      });
      this.hexMap.onHexClick = (hex) => {
        this.onHexClick(hex);
      };
      this.hexMap.onHexHover = (hex) => {
        if (hex) {
          this.updateInfoPanel(`Hex ${hex.x},${hex.y} - ${hex.terrain || "Unknown"}`);
        } else {
          this.updateInfoPanel("No hex selected");
        }
      };
      console.log("\u2705 Hexcrawl View: Custom hex map created successfully");
    } catch (error) {
      console.error("\u274C Hexcrawl View: Failed to create custom hex map:", error);
      return;
    }
    console.log("\u{1F5FA}\uFE0F Hexcrawl View: Map initialized, waiting for game session");
  }
  async refreshMap() {
    var _a;
    console.log("\u{1F504} Hexcrawl View: Refreshing map...");
    console.log("\u{1F50D} Debug - Current Session ID:", this.plugin.currentSessionId);
    console.log("\u{1F50D} Debug - Plugin State:", {
      isAuthenticated: this.plugin.authState.isAuthenticated,
      hasUser: !!this.plugin.authState.user,
      hasApiClient: !!this.plugin.apiClient,
      hasMapData: !!this.plugin.mapData
    });
    if (!this.plugin.currentSessionId) {
      console.log("\u26A0\uFE0F No session ID, cannot load map");
      new import_obsidian.Notice("No active session. Please join a session first.");
      return;
    }
    await this.plugin.loadMapData();
    if (this.plugin.mapData && this.hexMap) {
      console.log("\u{1F5FA}\uFE0F Hexcrawl View: Updating map data with", ((_a = this.plugin.mapData.hexes) == null ? void 0 : _a.length) || 0, "hexes");
      this.hexMap.setMapData(this.plugin.mapData);
      this.updateStatusDisplays();
      if (this.plugin.mapData.player_position) {
        this.hexMap.centerOnPlayer();
      }
    } else {
      console.log("\u26A0\uFE0F No map data or hex map available");
      if (!this.hexMap) {
        console.error("\u274C Hex map not initialized!");
      }
    }
  }
  updateStatusDisplays() {
  }
  async onHexClick(hex) {
    var _a;
    let info = `<h4>Hex ${hex.x},${hex.y}</h4>`;
    info += `<p><strong>Terrain:</strong> ${hex.terrain || "Unknown"}</p>`;
    if (hex.biome)
      info += `<p><strong>Biome:</strong> ${hex.biome}</p>`;
    if (hex.elevation)
      info += `<p><strong>Elevation:</strong> ${hex.elevation}</p>`;
    if (hex.description)
      info += `<p><strong>Description:</strong> ${hex.description}</p>`;
    this.updateInfoPanel(info);
    if ((_a = this.plugin.mapData) == null ? void 0 : _a.player_position) {
      const playerPos = this.plugin.mapData.player_position;
      const currentHex = { x: playerPos.x, y: playerPos.y };
      const targetQ = hex.x;
      const targetR = hex.y;
      const targetS = -targetQ - targetR;
      const currentQ = currentHex.x;
      const currentR = currentHex.y;
      ...
      if (isAdjacent) {
        ...
        console.log(`\u{1F4CD} Hex ${hex.x},${hex.y} is not adjacent to player position (${currentHex.x}, ${currentHex.y})`);
      }
    }
  }
  isAdjacent(q1, r1, s1, q2, r2, s2) {
    const distance = Math.max(Math.abs(q1 - q2), Math.abs(r1 - r2), Math.abs(s1 - s2));
    return distance === 1;
  }
  async onClose() {
    if (this.hexMap) {
...
var HexMoveConfirmModal = class extends import_obsidian.Modal {
  ...
  showNormalState() {
    const { contentEl } = this;
    contentEl.createEl("h2", { text: "Move to this hex?" });
    const infoDiv = contentEl.createDiv({ cls: "hex-move-info" });
    infoDiv.createEl("p", { text: `Location: (${this.hex.x}, ${this.hex.y})` });
    if (this.hex.terrain) {
      infoDiv.createEl("p", { text: `Terrain: ${this.hex.terrain}` });
    }
    if (this.hex.biome) {
      infoDiv.createEl("p", { text: `Biome: ${this.hex.biome}` });
    }
    const buttonContainer = contentEl.createDiv({ cls: "hex-move-buttons" });
  ...
  async goToHexNote() {
    var _a, _b;
    const hexNotesFolder = ((_b = (_a = this.plugin) == null ? void 0 : _a.settings) == null ? void 0 : _b.hexNotesFolder) || "Hexcrawl/Hexes";
    const hexName = `Hex_${this.hex.x}_${this.hex.y}`;
    const notePath = `${hexNotesFolder}/${hexName}.md`;
    const file = this.app.vault.getAbstractFileByPath(notePath);
    if (file) {
      await this.app.workspace.openLinkText(notePath, "", false);
    } else {
      const folder = this.app.vault.getAbstractFileByPath(hexNotesFolder);
      if (!folder) {
        await this.app.vault.createFolder(hexNotesFolder);
      }
      let content = `# ${hexName}

`;
      content += `## Location
- Coordinates: (${this.hex.x}, ${this.hex.y})
`;
      if (this.hex.terrain)
        content += `- Terrain: ${this.hex.terrain}
`;
      if (this.hex.biome)
        content += `- Biome: ${this.hex.biome}
`;
      if (this.hex.elevation)
        content += `- Elevation: ${this.hex.elevation}
`;
      content += `
## Description
${this.hex.description || "No description yet."}
`;
      content += `
## Notes
...
`;
  ...
  showWaitingState() {
    const { contentEl } = this;
    contentEl.createEl("h2", { text: "Waiting for Master Approval" });
    const waitingDiv = contentEl.createDiv({ cls: "hex-move-waiting" });
    waitingDiv.style.textAlign = "center";
    waitingDiv.style.padding = "20px";
    const spinner = waitingDiv.createEl("div", { cls: "spinner" });
    ...
    waitingDiv.createEl("p", { text: "Your movement request has been sent to the Master." });
    waitingDiv.createEl("p", { text: "Please wait for approval..." });
    ...
    buttonContainer.style.marginTop = "20px";
    new import_obsidian.ButtonComponent(buttonContainer).setButtonText("Cancel").onClick(() => {
      this.cancelMovementRequest();
    });
  }
  async requestMovement() {
    var _a, _b;
    const targetQ = this.hex.x;
    const targetR = this.hex.y;
    const targetS = -targetQ - targetR;
    try {
      const response = await ((_a = this.plugin.apiClient) == null ? void 0 : _a.post("/api/request_move", {
        session_id: this.plugin.currentSessionId,
        q: targetQ,
        r: targetR,
        s: targetS
      }));
      if ((_b = response == null ? void 0 : response.data) == null ? void 0 : _b.success) {
        this.isWaitingForApproval = true;
        this.onOpen();
        this.startApprovalCheck();
      } else {
        new import_obsidian.Notice("Failed to send movement request");
        this.close();
      }
    } catch (error) {
      console.error("Failed to request movement:", error);
      new import_obsidian.Notice("Failed to send movement request");
      this.close();
    }
  }
  ...
  stopApprovalCheck() {
    if (this.approvalCheckInterval) {
      window.clearInterval(this.approvalCheckInterval);
      this.approvalCheckInterval = null;
    }
  }
  async cancelMovementRequest() {
    var _a;
    try {
      await ((_a = this.plugin.apiClient) == null ? void 0 : _a.post("/api/cancel_movement_request", {
        session_id: this.plugin.currentSessionId
      }));
    } catch (error) {
      console.error("Failed to cancel movement request:", error);
    }
    this.stopApprovalCheck();
    this.close();
  }
...
var VIEW_TYPE_AUTH = "hexcrawl-auth";
var AuthView = class extends import_obsidian2.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    ...
    this.plugin = plugin;
    console.log("\u{1F510} Auth View: Constructor called");
  }
  getViewType() {
    return VIEW_TYPE_AUTH;
  }
  getDisplayText() {
    return "Hexcrawl Authentication";
  }
  getIcon() {
    return "user";
  }
  async onOpen() {
    console.log("\u{1F510} Auth View: Opening authentication view");
    if (this.plugin.authState.isAuthenticated) {
  ...
  render() {
    const container = this.containerEl.children[1];
    container.empty();
    container.addClass("hexcrawl-auth-view");
    const header = container.createDiv({ cls: "auth-header" });
    header.createEl("h2", { text: "Hexcrawl Explorer" });
    if (this.plugin.authState.isAuthenticated) {
      this.renderProfile(container);
    } else {
      this.renderAuthTabs(container);
  ...
  renderAuthTabs(container) {
    const tabsDiv = container.createDiv({ cls: "auth-tabs" });
    const loginTab = tabsDiv.createEl("button", {
      text: "Login",
      cls: this.currentView === "login" ? "auth-tab active" : "auth-tab"
    });
    const registerTab = tabsDiv.createEl("button", {
      text: "Create Account",
      cls: this.currentView === "register" ? "auth-tab active" : "auth-tab"
    });
    loginTab.onclick = () => {
      this.currentView = "login";
      this.render();
  ...
  renderLogin(container) {
    ...
    formDiv.createEl("h3", { text: "Login to Your Account" });
    this.loginForm = {
      username: new import_obsidian2.TextComponent(formDiv),
      password: new import_obsidian2.TextComponent(formDiv),
      remember: null
    };
    const usernameSetting = new import_obsidian2.Setting(formDiv).setName("Username").setDesc("Enter your username");
    this.loginForm.username.inputEl.placeholder = "Username";
    this.loginForm.username.inputEl.type = "text";
    usernameSetting.controlEl.appendChild(this.loginForm.username.inputEl);
    const passwordSetting = new import_obsidian2.Setting(formDiv).setName("Password").setDesc("Enter your password");
    this.loginForm.password.inputEl.placeholder = "Password";
    this.loginForm.password.inputEl.type = "password";
    passwordSetting.controlEl.appendChild(this.loginForm.password.inputEl);
    const rememberSetting = new import_obsidian2.Setting(formDiv).setName("Remember me").setDesc("Stay logged in");
    this.loginForm.remember = rememberSetting.controlEl.createEl("input", {
      type: "checkbox"
    });
    const buttonDiv = formDiv.createDiv({ cls: "auth-buttons" });
    new import_obsidian2.ButtonComponent(buttonDiv).setButtonText("Login").setCta().onClick(() => this.handleLogin());
  ...
  renderRegister(container) {
    ...
    usernameSetting.controlEl.appendChild(this.registerForm.username.inputEl);
    const emailSetting = new import_obsidian2.Setting(formDiv).setName("Email").setDesc("Enter your email address");
    this.registerForm.email.inputEl.placeholder = "email@example.com";
    this.registerForm.email.inputEl.type = "email";
    emailSetting.controlEl.appendChild(this.registerForm.email.inputEl);
    const passwordSetting = new import_obsidian2.Setting(formDiv).setName("Password").setDesc("Choose a strong password (minimum 8 characters)");
    this.registerForm.password.inputEl.placeholder = "Password";
    this.registerForm.password.inputEl.type = "password";
    passwordSetting.controlEl.appendChild(this.registerForm.password.inputEl);
    const confirmSetting = new import_obsidian2.Setting(formDiv).setName("Confirm Password").setDesc("Re-enter your password");
  ...
  renderProfile(container) {
    const user = this.plugin.authState.user;
    const profileDiv = container.createDiv({ cls: "auth-profile" });
    profileDiv.createEl("h3", { text: `Welcome, ${user.username}!` });
    const infoDiv = profileDiv.createDiv({ cls: "user-info" });
    const colorPreview = infoDiv.createDiv({ cls: "color-preview" });
    colorPreview.style.backgroundColor = user.playerColor;
    colorPreview.style.width = "20px";
    colorPreview.style.height = "20px";
    colorPreview.style.borderRadius = "50%";
    colorPreview.style.display = "inline-block";
    colorPreview.style.marginRight = "10px";
    infoDiv.createSpan({ text: `Player Color: ${user.playerColor}` });
    infoDiv.createEl("br");
    infoDiv.createSpan({ text: `Email: ${user.email}` });
    if (user.isGameMaster) {
      infoDiv.createEl("br");
      infoDiv.createSpan({ text: "\u{1F3B2} Game Master", cls: "gm-badge" });
    }
    this.profileForm = {
    ...
    profileDiv.createEl("h4", { text: "Update Profile" });
    ...
    this.profileForm.currentPassword.inputEl.type = "password";
    this.profileForm.currentPassword.inputEl.placeholder = "Current Password";
    currentPassSetting.controlEl.appendChild(this.profileForm.currentPassword.inputEl);
    const newPassSetting = new import_obsidian2.Setting(profileDiv).setName("New Password").setDesc("Enter a new password (optional)");
    this.profileForm.newPassword.inputEl.type = "password";
    this.profileForm.newPassword.inputEl.placeholder = "New Password";
    newPassSetting.controlEl.appendChild(this.profileForm.newPassword.inputEl);
    const confirmPassSetting = new import_obsidian2.Setting(profileDiv).setName("Confirm New Password").setDesc("Re-enter your new password");
    this.profileForm.confirmPassword.inputEl.type = "password";
    this.profileForm.confirmPassword.inputEl.placeholder = "Confirm New Password";
    confirmPassSetting.controlEl.appendChild(this.profileForm.confirmPassword.inputEl);
    const buttonDiv = profileDiv.createDiv({ cls: "auth-buttons" });
    new import_obsidian2.ButtonComponent(buttonDiv).setButtonText("Update Profile").setCta().onClick(() => this.handleUpdateProfile());
    new import_obsidian2.ButtonComponent(buttonDiv).setButtonText("Logout").onClick(() => this.handleLogout());
    this.renderSessionManagement(container);
  }
  renderSessionManagement(container) {
    const sessionDiv = container.createDiv({ cls: "session-management" });
    sessionDiv.createEl("h3", { text: "Game Session" });
    const statusDiv = sessionDiv.createDiv({ cls: "session-status" });
    if (this.plugin.currentSessionId) {
      statusDiv.createEl("p", {
        text: `\u{1F3AE} Connected to Session`,
        cls: "success-text"
      });
      const sessionContainer = statusDiv.createDiv({ cls: "session-id-container" });
      sessionContainer.style.display = "flex";
      sessionContainer.style.alignItems = "center";
      sessionContainer.style.gap = "10px";
      sessionContainer.createEl("span", {
        text: "Session ID:",
        cls: "setting-item-description"
      });
      new import_obsidian2.ButtonComponent(sessionContainer).setButtonText("Copy Session ID").setIcon("copy").onClick(() => {
        navigator.clipboard.writeText(this.plugin.currentSessionId || "");
        new import_obsidian2.Notice("Session ID copied to clipboard!");
      });
      new import_obsidian2.ButtonComponent(statusDiv).setButtonText("Leave Session").onClick(() => {
        this.plugin.currentSessionId = null;
        this.plugin.mapData = null;
        new import_obsidian2.Notice("Left session");
        this.render();
      });
    } else {
      statusDiv.createEl("p", {
      ...
      statusDiv.createEl("p", {
        text: "Get the Session ID from the Game Master in the web interface",
        cls: "setting-item-description"
      });
    }
    sessionDiv.createEl("h4", { text: "Join Master Session" });
    sessionDiv.createEl("p", {
      text: "Enter the Session ID provided by your Game Master",
      cls: "setting-item-description"
    });
    const joinDiv = sessionDiv.createDiv({ cls: "session-join" });
    const sessionIdInput = new import_obsidian2.TextComponent(joinDiv);
    sessionIdInput.setPlaceholder("Enter Session ID");
    if (this.plugin.currentSessionId) {
      sessionIdInput.setValue("");
    }
    new import_obsidian2.ButtonComponent(joinDiv).setButtonText("Join Session").setCta().onClick(async () => {
      var _a, _b, _c, _d, _e;
      const sessionId = sessionIdInput.getValue().trim();
      if (!sessionId) {
        new import_obsidian2.Notice("Please enter a Session ID");
        return;
      }
      try {
        console.log("\u{1F510} Checking session debug before joining...");
        try {
          const debugCheck = await this.plugin.apiClient.get("/api/auth/session-debug");
          console.log("\u{1F4CA} Session debug:", debugCheck.data);
        } catch (debugError) {
          console.error("\u274C Session debug failed:", debugError);
        }
        console.log("\u{1F510} Checking authentication before joining session...");
        try {
          const authCheck = await this.plugin.apiClient.get("/api/auth/me");
          console.log("\u2705 Authentication verified:", authCheck.data);
        } catch (authError) {
          console.error("\u274C Authentication failed:", authError);
          new import_obsidian2.Notice("\u274C Authentication expired. Please log in again.");
          this.plugin.authState = { isAuthenticated: false, user: null };
          this.render();
          return;
        }
        ...
        if (gameResponse.data.success || gameResponse.data.session_id) {
          ...
          const hexView = this.plugin.app.workspace.getLeavesOfType(VIEW_TYPE_HEXCRAWL)[0];
          if (hexView && hexView.view instanceof HexcrawlView) {
            console.log("\u{1F5FA}\uFE0F Refreshing already open hex view");
            await hexView.view.refreshMap();
          }
        } else {
          new import_obsidian2.Notice(`\u274C Failed to create player session for: ${sessionId}`);
        }
      } catch (error) {
        console.error("Failed to join session:", error);
        if (((_d = error.response) == null ? void 0 : _d.status) === 404) {
          new import_obsidian2.Notice(`\u274C Session not found: ${sessionId}`);
        } else if (((_e = error.response) == null ? void 0 : _e.status) === 403) {
          new import_obsidian2.Notice("\u274C Access denied to this session");
        } else {
          new import_obsidian2.Notice("\u274C Failed to join session - check Session ID");
        }
      }
    });
  }
  renderServerStatus(container) {
    const statusDiv = container.createDiv({ cls: "server-status" });
    statusDiv.createEl("h4", { text: "Server Connection" });
    const statusText = statusDiv.createEl("p", { text: "Checking connection..." });
    this.plugin.testServerConnection().then((isConnected) => {
      if (isConnected) {
        statusText.setText("\u2705 Connected to server");
        statusText.style.color = "var(--text-success)";
      } else {
        statusText.setText("\u274C Cannot connect to server");
        statusText.style.color = "var(--text-error)";
      }
    });
    statusDiv.createEl("p", {
      text: `Server: ${this.plugin.settings.serverUrl}`,
      cls: "server-url"
    });
  }
  async handleLogin() {
    if (!this.loginForm)
      return;
    const credentials = {
      username: this.loginForm.username.getValue().trim(),
      password: this.loginForm.password.getValue(),
      remember: this.loginForm.remember.checked
    };
    ...
    try {
      const success = await this.plugin.login(credentials);
      if (success) {
        new import_obsidian2.Notice(`Welcome back, ${credentials.username}!`);
        this.render();
      }
    } catch (error) {
      console.error("Login failed:", error);
      new import_obsidian2.Notice("\u26A0\uFE0F Login failed - Check credentials & server connection");
    }
  }
  async handleRegister() {
    ...
    if (!registration.username || !registration.email || !registration.password) {
      new import_obsidian2.Notice("Please fill in all required fields");
      return;
    }
    ...
    if (registration.password.length < 8) {
      new import_obsidian2.Notice("Password must be at least 8 characters long");
      return;
    }
    if (registration.username.length < 3) {
      new import_obsidian2.Notice("Username must be at least 3 characters long");
      return;
    }
    try {
      const success = await this.plugin.registerUser(registration);
      if (success) {
        new import_obsidian2.Notice(`Account created successfully! Welcome, ${registration.username}!`);
        this.render();
      }
    } catch (error) {
      console.error("Registration failed:", error);
      new import_obsidian2.Notice("\u26A0\uFE0F Registration failed - Check server connection");
    }
  }
  async handleUpdateProfile() {
    if (!this.profileForm)
      return;
    const currentPassword = this.profileForm.currentPassword.getValue();
    if (!currentPassword) {
      new import_obsidian2.Notice("Please enter your current password to make changes");
      return;
    }
    const updates = {
      email: this.profileForm.email.getValue().trim(),
      playerColor: this.profileForm.playerColor.getValue(),
      currentPassword
    };
    const newPassword = this.profileForm.newPassword.getValue();
    const confirmPassword = this.profileForm.confirmPassword.getValue();
    if (newPassword) {
      if (newPassword !== confirmPassword) {
        new import_obsidian2.Notice("New passwords do not match");
        return;
      }
      if (newPassword.length < 8) {
        new import_obsidian2.Notice("New password must be at least 8 characters long");
        return;
      }
      updates.newPassword = newPassword;
    }
    try {
      const success = await this.plugin.updateProfile(updates);
      if (success) {
        new import_obsidian2.Notice("Profile updated successfully!");
        this.profileForm.currentPassword.setValue("");
        this.profileForm.newPassword.setValue("");
        this.profileForm.confirmPassword.setValue("");
      }
    } catch (error) {
      console.error("Profile update failed:", error);
      new import_obsidian2.Notice("\u26A0\uFE0F Profile update failed - Check current password");
    }
  }
  async handleLogout() {
    try {
      ...
      new import_obsidian2.Notice("\u26A0\uFE0F Logout failed - Check server connection");
    }
  }
  async onClose() {
  }
...
var DEFAULT_SETTINGS = {
  ...
  hexNotesFolder: "Hexcrawl/Locations",
...
(function(ErrorCode2) {
  ...
  ErrorCode2[ErrorCode2["MissingTargetTypeHeader"] = 40053] = "MissingTargetTypeHeader";
  ErrorCode2[ErrorCode2["InvalidTargetTypeHeader"] = 40054] = "InvalidTargetTypeHeader";
  ErrorCode2[ErrorCode2["MissingTargetHeader"] = 40055] = "MissingTargetHeader";
  ErrorCode2[ErrorCode2["MissingOperation"] = 40056] = "MissingOperation";
  ErrorCode2[ErrorCode2["InvalidOperation"] = 40057] = "InvalidOperation";
...
var HexcrawlPlugin = class extends import_obsidian4.Plugin {
  ...
  async onload() {
    console.log("\u{1F680} Hexcrawl Plugin: Starting to load...");
    try {
      await this.loadSettings();
      console.log("\u2699\uFE0F Hexcrawl Plugin: Settings loaded:", this.settings);
      ...
      this.addRibbonIcon("map", "Hexcrawl Explorer", () => {
      ...
      this.addCommand({
        id: "open-hexcrawl-auth",
        name: "Open Hexcrawl Login",
        callback: () => {
          this.activateAuthView();
        }
      });
      this.addCommand({
        id: "open-hexcrawl-view",
        name: "Open Hexcrawl Explorer",
        callback: () => {
          if (this.authState.isAuthenticated) {
            this.activateView();
          } else {
      ...
      this.addCommand({
        id: "sync-hex-to-note",
        name: "Sync Current Hex to Note",
        callback: () => {
          this.syncCurrentHexToNote();
        }
      });
      this.addCommand({
        id: "generate-text-mapper",
        name: "Generate Text-Mapper Notation",
        callback: async () => {
      ...
      this.addCommand({
        id: "test-cors",
        name: "Test CORS Connection",
        callback: () => {
          this.testCorsConnection();
        }
      });
      this.addSettingTab(new HexcrawlSettingTab(this.app, this));
      if (this.settings.autoLogin) {
        console.log("\u{1F504} Hexcrawl Plugin: Checking for saved login...");
        await this.checkSavedLogin();
      }
      if (this.authState.isAuthenticated && this.settings.lastSessionId) {
        console.log("\u{1F3AE} Hexcrawl Plugin: Rejoining last session:", this.settings.lastSessionId);
        try {
          const response = await this.apiClient.get(`/api/load_map_session/${this.settings.lastSessionId}`);
          if (response.data.success) {
            this.currentSessionId = this.settings.lastSessionId;
            this.mapData = response.data.map_data;
            console.log("\u2705 Rejoined session:", this.settings.lastSessionId);
          }
        } catch (error) {
          console.log("\u26A0\uFE0F Could not rejoin last session");
        }
      }
      console.log("\u2705 Hexcrawl Plugin: Successfully loaded!");
    } catch (error) {
      console.error("\u274C Hexcrawl Plugin: Failed to load:", error);
    }
  }
  onunload() {
  }
  initializeApiClient() {
    ...
    console.log("\u{1F527} Hexcrawl: Initializing API client with URL:", this.settings.serverUrl);
    console.trace("\u{1F50D} Stack trace for initializeApiClient call:");
    console.log("\u{1F50D} Token restoration debug:");
    console.log("  authState.token:", ((_a = this.authState) == null ? void 0 : _a.token) ? "[PRESENT]" : "[MISSING]");
    console.log("  current Authorization header:", ((_d = (_c = (_b = this.apiClient) == null ? void 0 : _b.defaults) == null ? void 0 : _c.headers) == null ? void 0 : _d["Authorization"]) ? "[PRESENT]" : "[MISSING]");
    console.log("  savedToken in settings:", this.settings.savedToken ? "[PRESENT]" : "[MISSING]");
    let existingToken = null;
    if ((_e = this.authState) == null ? void 0 : _e.token) {
      existingToken = `Bearer ${this.authState.token}`;
      console.log("  Using token from authState");
    } else if ((_h = (_g = (_f = this.apiClient) == null ? void 0 : _f.defaults) == null ? void 0 : _g.headers) == null ? void 0 : _h["Authorization"]) {
      existingToken = this.apiClient.defaults.headers["Authorization"];
      console.log("  Using token from current header");
    } else if (this.settings.savedToken) {
      existingToken = `Bearer ${atob(this.settings.savedToken)}`;
      console.log("  Using token from saved settings");
    }
    console.log("  final existingToken:", existingToken ? "[PRESENT]" : "[MISSING]");
    this.apiClient = axios_default.create({
    ...
    if (existingToken) {
      ...
      console.log("\u{1F511} Restored JWT token to new API client");
    } else {
      console.log("\u26A0\uFE0F No token available to restore");
    }
    setTimeout(() => {
      var _a2;
      if (((_a2 = this.authState) == null ? void 0 : _a2.token) && !this.apiClient.defaults.headers["Authorization"]) {
        console.log("\u{1F504} Failsafe: Restoring lost token from authState");
        this.apiClient.defaults.headers["Authorization"] = `Bearer ${this.authState.token}`;
      }
    }, 100);
    this.apiClient.interceptors.request.use((config) => {
      var _a2;
      console.log("\u{1F4E4} Hexcrawl API Request:", {
        method: (_a2 = config.method) == null ? void 0 : _a2.toUpperCase(),
        url: config.url,
        data: config.data,
      ...
      console.error("\u{1F4E4}\u274C Hexcrawl API Request Error:", error);
      return Promise.reject(error);
    });
    this.apiClient.interceptors.response.use((response) => {
      console.log("\u{1F4E5}\u2705 Hexcrawl API Response:", {
        status: response.status,
      ...
      var _a2, _b2, _c2, _d2;
      console.error("\u{1F4E5}\u274C Hexcrawl API Response Error:", {
        status: (_a2 = error.response) == null ? void 0 : _a2.status,
        statusText: (_b2 = error.response) == null ? void 0 : _b2.statusText,
        url: (_c2 = error.config) == null ? void 0 : _c2.url,
        message: error.message,
        data: (_d2 = error.response) == null ? void 0 : _d2.data
      });
      return Promise.reject(error);
    });
  }
  async activateView() {
    ...
    let leaf = null;
    const leaves = workspace.getLeavesOfType(VIEW_TYPE_HEXCRAWL);
    if (leaves.length > 0) {
      leaf = leaves[0];
    } else {
      leaf = workspace.getRightLeaf(false);
      if (leaf) {
        await leaf.setViewState({ type: VIEW_TYPE_HEXCRAWL, active: true });
      }
    }
    if (leaf) {
  ...
  async activateAuthView() {
    ...
    const { workspace } = this.app;
    let leaf = null;
    const leaves = workspace.getLeavesOfType(VIEW_TYPE_AUTH);
    if (leaves.length > 0) {
      console.log("\u{1F510} Auth View: Found existing leaf");
      leaf = leaves[0];
    } else {
      console.log("\u{1F510} Auth View: Creating new leaf in main workspace");
      leaf = workspace.getLeaf(true);
      if (leaf) {
        console.log("\u{1F510} Auth View: Setting view state on main leaf");
    ...
    if (leaf) {
      console.log("\u{1F510} Auth View: Revealing leaf");
      workspace.revealLeaf(leaf);
    } else {
  ...
  async saveSettings() {
    var _a, _b;
    const currentApiBaseUrl = (_b = (_a = this.apiClient) == null ? void 0 : _a.defaults) == null ? void 0 : _b.baseURL;
    console.log("\u{1F4BE} SaveSettings debug:", {
      currentApiBaseUrl,
      settingsServerUrl: this.settings.serverUrl,
      areEqual: currentApiBaseUrl === this.settings.serverUrl
    });
    await this.saveData(this.settings);
    if (currentApiBaseUrl !== this.settings.serverUrl) {
      console.log("\u{1F504} Server URL changed, reinitializing API client...");
      this.initializeApiClient();
    } else {
      console.log("\u{1F4BE} Settings saved without reinitializing API client (URL unchanged)");
    }
  }
  async testServerConnection() {
    try {
      const response = await this.apiClient.get("/api/plugin/health");
      return response.status === 200;
    } catch (error) {
      console.error("\u{1F517} Server connection test failed:", error);
      return false;
    }
  }
  async checkSavedLogin() {
    try {
      if (this.settings.savedToken) {
        const restoredToken = atob(this.settings.savedToken);
        this.apiClient.defaults.headers["Authorization"] = `Bearer ${restoredToken}`;
        this.authState = {
          isAuthenticated: false,
          user: null,
          token: restoredToken
        };
        console.log("\u{1F511} Restored saved JWT token");
      }
      ...
      if (response.data.user) {
        ...
        if (token) {
          ...
          console.log("\u{1F511} JWT token set for future requests");
        }
        this.authState = {
  ...
  async login(credentials) {
    var _a, _b;
    try {
      console.log("\u{1F510} Attempting login for:", credentials.username);
      ...
      if (response.data.user) {
        ...
        if (credentials.remember) {
          this.settings.rememberLogin = true;
          this.settings.autoLogin = true;
          this.settings.savedToken = token ? btoa(token) : void 0;
          console.log("\u{1F4BE} Saving login preferences and token...");
          await this.saveSettings();
          console.log("\u2705 Settings saved, checking token preservation...");
          const currentAuth = this.apiClient.defaults.headers["Authorization"];
          console.log("\u{1F50D} Current Authorization header after saveSettings:", currentAuth);
          if (!currentAuth && token) {
            console.log("\u26A0\uFE0F Token lost after saveSettings, restoring...");
            this.apiClient.defaults.headers["Authorization"] = `Bearer ${token}`;
            console.log("\u{1F504} Token restored manually");
          }
        }
        console.log("\u2705 Login successful:", user.username);
        try {
          const debugCheck = await this.apiClient.get("/api/auth/session-debug");
          console.log("\u{1F4CA} Post-login session debug:", debugCheck.data);
        } catch (debugError) {
          console.error("\u274C Post-login session debug failed:", debugError);
        }
        return true;
      }
      return false;
      ...
      if (error.code === "ERR_NETWORK") {
        throw new Error("\u{1F310} Server unreachable - Check server is running");
      } else if (((_a = error.response) == null ? void 0 : _a.status) === 401) {
        throw new Error("\u{1F511} Invalid username or password");
      } else if (((_b = error.response) == null ? void 0 : _b.status) === 429) {
        throw new Error("\u23F0 Too many attempts - Try again later");
      } else {
        throw new Error("\u26A0\uFE0F Login failed - Check connection");
      }
    }
  }
  async registerUser(registration) {
    var _a, _b, _c;
    try {
      console.log("\u{1F4DD} Attempting registration for:", registration.username);
      ...
      if (error.code === "ERR_NETWORK") {
        ...
        throw new Error("\u{1F465} Username or email already taken");
      } else if ((_c = (_b = error.response) == null ? void 0 : _b.data) == null ? void 0 : _c.errors) {
        const errors = error.response.data.errors;
        const errorMessages = Object.values(errors).flat();
        throw new Error(`\u{1F4DD} ${errorMessages.join(", ")}`);
      } else {
        throw new Error("\u26A0\uFE0F Registration failed - Try again");
      }
    }
  }
  async logout() {
    ...
    this.currentSessionId = null;
    this.mapData = null;
    this.settings.rememberLogin = false;
    this.settings.autoLogin = false;
    this.settings.savedToken = void 0;
    await this.saveSettings();
    console.log("\u{1F512} Logged out successfully");
  }
  async updateProfile(updates) {
    var _a, _b, _c;
    try {
      console.log("\u{1F464} Updating profile...");
      const response = await this.apiClient.post("/api/auth/update-profile", updates);
      if (response.data.user) {
        const user = {
          ...response.data.user,
          playerColor: response.data.user.color || response.data.user.playerColor
  ...
  async createNewGame(mapName = "default", seed) {
    ...
    try {
      ...
      if ((_a = this.authState.user) == null ? void 0 : _a.username)
        requestData.player_name = this.authState.user.username;
      if ((_b = this.authState.user) == null ? void 0 : _b.playerColor)
  ...
  async loadMapData() {
    var _a;
    if (!this.authState.isAuthenticated) {
      console.log("\u{1F4CA} Cannot load map data: Not authenticated");
      return;
    }
    if (!this.currentSessionId) {
      console.log("\u{1F4CA} Cannot load map data: No active session");
      return;
    }
    try {
      console.log(`\u{1F4CA} Loading map data for session: ${this.currentSessionId}`);
      const response = await this.apiClient.get(`/api/get_map?session_id=${this.currentSessionId}`);
      if (response.data.success) {
        const mapData = response.data.map_data || {};
        console.log("\u{1F50D} DEBUG - Raw map data from server:", JSON.stringify(mapData, null, 2));
        const hexes = (mapData.hexes || []).map((hex) => ({
          ...hex,
          x: hex.q !== void 0 ? hex.q : hex.x || 0,
          y: hex.r !== void 0 ? hex.r : hex.y || 0
        }));
        const playerPos = mapData.current_position || mapData.player_position || { q: 0, r: 0, s: 0 };
        console.log("\u{1F50D} DEBUG - Player position from server:", playerPos);
        const convertedPlayerPos = {
          x: playerPos.q !== void 0 ? playerPos.q : playerPos.x || 0,
          y: playerPos.r !== void 0 ? playerPos.r : playerPos.y || 0
        };
        console.log("\u{1F50D} DEBUG - Converted player position:", convertedPlayerPos);
        this.mapData = {
          hexes,
          player_position: convertedPlayerPos,
          map_name: mapData.map_name || "Hexcrawl Map",
          width: mapData.width || 50,
          height: mapData.height || 50
        };
        console.log("\u{1F4CA} Map data loaded successfully:", this.mapData);
      } else {
        console.error("\u274C Failed to load map data:", response.data);
      }
    } catch (error) {
  ...
  async moveToHex(q, r, s) {
    ...
    try {
      console.log(`\u{1F680} Moving to hex: (${q}, ${r}, ${s})`);
      const response = await this.apiClient.post(`/api/move?session_id=${this.currentSessionId}`, {
        q,
        r,
        s
      });
      if (response.data.success) {
        if (response.data.map_data) {
          ...
          const playerPos = mapData.current_position || mapData.player_position || { q, r, s };
          const convertedPlayerPos = {
            x: playerPos.q !== void 0 ? playerPos.q : playerPos.x || 0,
            y: playerPos.r !== void 0 ? playerPos.r : playerPos.y || 0
          };
          this.mapData = {
            hexes,
            player_position: convertedPlayerPos,
            map_name: ((_a = this.mapData) == null ? void 0 : _a.map_name) || "Hexcrawl Map",
            width: ((_b = this.mapData) == null ? void 0 : _b.width) || 50,
            height: ((_c = this.mapData) == null ? void 0 : _c.height) || 50
          };
        } else {
          await this.loadMapData();
        }
  ...
  async movePlayer(direction) {
    ...
    try {
      const currentQ = this.mapData.player_position.x;
      const currentR = this.mapData.player_position.y;
      const currentS = -currentQ - currentR;
      ...
      const targetR = currentR + offset.r;
      const targetS = currentS + offset.s;
      const response = await this.apiClient.post(`/api/move?session_id=${this.currentSessionId}`, {
        q: targetQ,
        r: targetR,
        s: targetS
  ...
  async generateHexDescription(x, y) {
    var _a;
    if (!this.authState.isAuthenticated) {
      return "";
    }
    try {
      ...
      const r = y - (x - (x & 1)) / 2;
      const s = -q - r;
      const response = await this.apiClient.post("/api/generate_description", {
        q,
        r,
        s
      });
      return response.data.description || response.data.result || "";
    } catch (error) {
      console.error("Failed to generate hex description:", error);
      if (((_a = error.response) == null ? void 0 : _a.status) === 401) {
        this.authState = { isAuthenticated: false, user: null };
      }
  ...
  generateTextMapperNotation() {
    ...
    const exploredHexes = this.mapData.hexes.filter((hex) => hex.explored);
    if (exploredHexes.length === 0) {
      return "No explored hexes yet.";
    }
    const minX = Math.min(...exploredHexes.map((h) => h.x));
    const maxX = Math.max(...exploredHexes.map((h) => h.x));
    const minY = Math.min(...exploredHexes.map((h) => h.y));
    const maxY = Math.max(...exploredHexes.map((h) => h.y));
    let textMap = "";
    let hexNumber = 1;
    const hexLabels = [];
    for (let y = minY; y <= maxY; y++) {
      ...
      for (let x = minX; x <= maxX; x++) {
        const hex = exploredHexes.find((h) => h.x === x && h.y === y);
        if (hex) {
          const terrainCode = TERRAIN_MAP[((_a = hex.terrain) == null ? void 0 : _a.toLowerCase()) || ""] || "grass";
          const hexLabel = String(hexNumber).padStart(2, "0");
          row += hexLabel + " ";
          let label = `${hexLabel} ${terrainCode}`;
          if (hex.features && hex.features.length > 0) {
            label += ` (${hex.features.join(", ")})`;
          }
          if (hex.description) {
    ...
    const result = `# Explored Hexcrawl Map
...
${hexLabels.join("\n")}`;
  ...
  async createOrUpdateHexNote(hex) {
    ...
    let description = hex.description;
    if (!description && this.settings.autoGenerateDescriptions) {
      description = await this.generateHexDescription(hex.x, hex.y);
    }
  ...
  generateHexNoteContent(hex, description) {
    const content = [];
    content.push(`# Hex ${hex.x}-${hex.y}`);
    content.push("");
    content.push(`## Location Information`);
    content.push(`- **Coordinates:** ${hex.x}, ${hex.y}`);
    content.push(`- **Terrain:** ${hex.terrain || "Unknown"}`);
    if (hex.biome)
      content.push(`- **Biome:** ${hex.biome}`);
    if (hex.elevation)
      content.push(`- **Elevation:** ${hex.elevation}`);
    content.push("");
    if (description) {
    ...
    content.push(`## Notes`);
    content.push(`*Add your notes about this location here...*`);
    content.push("");
    content.push(`## Connected Hexes`);
    const directions = ["n", "ne", "se", "s", "sw", "nw"];
    directions.forEach((dir) => {
      const neighbor = this.getNeighborCoords(hex.x, hex.y, dir);
      content.push(`- **${dir.toUpperCase()}:** [[Hex ${neighbor.x}-${neighbor.y}]]`);
    });
    content.push("");
    content.push(`---`);
    content.push(`*Generated by Hexcrawl Explorer*`);
    return content.join("\n");
  }
  getNeighborCoords(x, y, direction) {
    const neighbors = {
      "n": { x, y: y - 1 },
      "ne": { x: x + 1, y: y - (x % 2 === 0 ? 1 : 0) },
      "se": { x: x + 1, y: y + (x % 2 === 0 ? 0 : 1) },
      "s": { x, y: y + 1 },
      "sw": { x: x - 1, y: y + (x % 2 === 0 ? 0 : 1) },
      "nw": { x: x - 1, y: y - (x % 2 === 0 ? 1 : 0) }
    };
    return neighbors[direction] || { x, y };
  }
  async copyDebugInfo() {
    var _a;
    const debugInfo = {
      plugin_version: this.manifest.version,
      obsidian_version: this.app.appVersion || "unknown",
      settings: this.settings,
      current_session: this.currentSessionId,
      has_map_data: !!this.mapData,
      map_data_size: this.mapData ? (_a = this.mapData.hexes) == null ? void 0 : _a.length : 0,
      api_base_url: this.apiClient.defaults.baseURL,
      timestamp: new Date().toISOString()
    };
    const debugText = `## Hexcrawl Plugin Debug Info
...
## Recent Console Logs
Check the developer console for detailed logs with \u{1F680}\u{1F527}\u{1F5FA}\uFE0F\u{1F4E4}\u{1F4E5} emoji prefixes.

## Steps to get console logs:
1. Press Ctrl+Shift+I (Windows) or Cmd+Option+I (Mac)
2. Click the Console tab
3. Try the action that's causing problems
...
`;
  ...
  async testCorsConnection() {
    var _a;
    console.log("\u{1F9EA} Testing CORS connection...");
    try {
...
var HexcrawlSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Hexcrawl Explorer Settings" });
    const authDiv = containerEl.createDiv({ cls: "user-status" });
    authDiv.createEl("h3", { text: "User Authentication" });
    authDiv.createEl("p", {
      text: "Click the user icon (\u{1F464}) in the left sidebar to login or create an account."
    });
    authDiv.createEl("p", {
      text: "Authentication is required to access hexcrawl features and sync with the server.",
      cls: "setting-item-description"
    });
